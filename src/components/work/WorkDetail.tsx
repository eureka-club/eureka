    ...
    if(posts){
      return <>
        ...
        {posts.map((p)=><Col key={p.id} xs={12} sm={6} lg={3} className="mb-5 d-flex justify-content-center  align-items-center">
          <MosaicItemPost  cacheKey={['POST',`${p.id}`]} postId={p.id} size={'md'} />          
        </Col>
        )}
        </Row>
  ...
  const renderCycles =()=>{
    if(cycles){
      return <Row className='mt-5'>
        {cycles.map(c=><Col xs={12} sm={6} lg={3} className="mb-5 d-flex justify-content-center  align-items-center" key={c.id}><CMI cycleId={c.id} cacheKey={['CYCLES',`WORK-${workId}`]} size={'md'} /></Col>)}
      </Row>
    }
    return <></> 
  }
            ...
            {post == null ? (
              <Row className="mb-5 d-flex flex-column flex-md-row">
                <Col className='col-md-5 col-lg-4 col-xl-3   d-none d-md-block'>
                  <MosaicItem workId={work.id} showTrash linkToWork={false} size={'lg'} />
                ...
                <Col className='col-md-7 col-lg-8 col-xl-9'>
                  ...
                  <div className='container d-sm-block d-md-none mt-4 mb-4 position-relative'>
                    <MosaicItem className='postition-absolute start-50 translate-middle-x'  workId={work.id} showTrash linkToWork={false} size={'lg'} />
                  </div>
