...
const PostDetail: FunctionComponent<Props> = ({ postId, work, cacheKey, showSaveForLater = false, session }) => {
  ...
  const currentUserIsParticipant = (user && [...user.cycles,...user.joinedCycles].findIndex((c) => c.id == +router.query.id!) >=0) || cycle?.creator.id == user?.id;
  ...
  if (!post && !isLoadingPost) return <Alert color="warning">Not found</Alert>;
  ...
  const handleExpandClick = () => {
    if (!session?.user) show(<SignInForm />);
  };
  ...
  return (
    <article data-cy="post-detail">
      <MosaicContext.Provider value={{ showShare: true }}>
        ...
        <h1 className="text-secondary fw-bold mb-2 d-sm-block d-lg-none"> {post?.title} </h1>{' '}
        {/*titulo y abajo texto eureka*/}
        ...
        <Stack direction={{ xs: 'column', lg: 'row' }}>
          ...
          <Box paddingLeft={3}>
            ...
            <h1 className="text-secondary fw-bold mb-2  d-none d-lg-block"> {post?.title} </h1>{' '}
            {/*titulo y abajo texto eureka*/}
