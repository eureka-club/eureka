...
import { FunctionComponent,FormEvent,ChangeEvent,useState,useEffect} from 'react';
...
const PostPrompt: FunctionComponent<Props> = ({redirect = false,searchtext,searchstyle}) => {
  ...
  const renderImages = ()=>{
    return <Container className="my-4"  >
     ...
     <section  className='d-flex flex-row justify-content-around'>
            {images.map((img,idx)=><img key={idx} src={img.src}/>)}
     </section> 
    </Container>
  }
       ...
       <form className='d-flex flex-row justify-content-center' onSubmit={onSubmit}>
         <TextField  label="Describe la imagen que quieres generar" required name="text"
          ...
          value={text}>
           ...
           <Button type='submit' className={`btn-eureka`} disabled={loading} style={{width:'15%',height:"3.5em"}}>
              Crear Imagen con IA
           </Button>
       </form>          
       </section>
