...
const SocialInteraction: FunctionComponent<Props> = ({
  ...
  const currentUserRating = cycle.ratings.findIndex(r=>r.userId==session?.user.id);
  ...
  useEffect(() => {
    ...
    if (session && cycle && cycle.favs) {
      ...
      ratingByMe = !!cycle.ratings.findIndex(r=>r.userId==session?.user.id);
      setMySocialInfo({ favoritedByMe, ratingByMe });
      const ratingCount = cycle.ratings.length;
  ...
  const shareUrl = `${WEBAPP_URL}/cycle/${cycle.id}`;
  const shareTextDynamicPart = `${t(dict,'cycleShare')} ${'title' in cycle ? `"${cycle.title}"` : ''}`;
    ...
    
    {
      mutationFn:async ({ socialInteraction, doCreate, ratingQty }: SocialInteractionClientPayload) => {
        if (session) {
          ...
          //   userName: user?.name,
          //   cycle: entityEndpoint.replace(/\w/, (c) => c.toUpperCase()),
          //   entityTitle: cycle.title,
          // })}`;
  
          // const notificationToUsers = user?.followedBy.map((f) => f.id);
          const res = await fetch(`/api/cycle/${cycle.id}/${socialInteraction}?lang=${lang}`, {
      ...
      onMutate: async (payload) => {
        if (session && user && payload.socialInteraction === 'fav') {
          ...
          if (!payload.doCreate) {
            ...
            // favInUser?.push(cycle as any);
            favs.push({ id: +session.user.id });
          }
          queryClient.setQueryData(['WORK', `${cycle.id}`], { ...cycle, favs });
  ...
  const handleCreateEurekaClick = (ev: MouseEvent<HTMLButtonElement>) => {
    ...
    if (canNavigate()) {
      ...
      router.push(`/cycle/${cycle.id}?tabKey=eurekas`);
    }
  };
  ...
  const getRatingLabelInfo = () => {
    return !currentUserRating ? <span className={styles.ratingLabelInfo}>{t(dict,'Rate it')}:</span> : '';
  };
  ...
  return (
    <section className={`${className} d-flex flex-row`}>
      ...
      <div className="ms-auto d-flex justify-content-end">
        ...
        {/* <div className={`ms-1`}>
        {
              isPost(cycle) && cycle?.reactions[0] 
               ? <span role="img" className="m-1" style={{verticalAlign:"sub"}} aria-label="emoji-ico" dangerouslySetInnerHTML={{__html: `${cycle?.reactions[0].emoji}`}} />
               : <></>
        }
        </div> */}
