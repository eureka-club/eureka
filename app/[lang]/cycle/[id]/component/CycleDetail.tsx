...
const CycleDetail: FunctionComponent<Props> = ({
  session
}) => {
    ...
    if(posts){
      return <>
        ...
        {posts.map((p)=><Col xs={12} sm={6} lg={3} xxl={2} key={p.id} className="mb-5 d-flex justify-content-center  align-items-center">
          <MosaicItemPost  cacheKey={['POST',`${p.id}`]} post={p} postId={p.id} showSaveForLater={false} size={'md'} />          
        </Col>
        )}
        </Row>
  ...
  const handleEditClick = (ev: MouseEvent<HTMLButtonElement>) => {
    ev.preventDefault();
    router.push(`/cycle/${cycle.id}/edit`);
  };
        ...
        <>
          ...
          <NavItem className={`cursor-pointer ${styles.tabBtn}`}>
            <NavLink eventKey="eurekas">
              <span className="mb-3">{t(dict,'EurekaMoments')} ({posts?.length})</span>
            </NavLink>
          </NavItem>
          {/* <NavItem className={`${styles.tabBtn}`}>
            <NavLink eventKey="my_milestone">{t(dict,'My milestones')}</NavLink>
          </NavItem> */}
          <NavItem className={`cursor-pointer ${styles.tabBtn}`}>
