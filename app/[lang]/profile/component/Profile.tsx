...
import LanguageSelectMultiple from '@/src/components/forms/controls/LanguageSelectMultiple';
...
const Profile: FunctionComponent<Props> = ({ user, session }) => {
      ...
      {user && (
        <Form onSubmit={handleSubmit}>
          ...
          <Row className="mt-4 d-flex flex-column flex-md-row">
            <Col>
              <TextField id="name" className="w-100 mt-5" label={`*${t(dict, 'Name')}`}
            ...
            <Col>
              <TextField id="email" className="w-100 mt-5" label={`*${t(dict, 'Email')}`}
            ...
            <Col className='mt-5'>
              <FormGroup controlId="countryOrigin" >
                <TagsInputTypeAheadMaterial
                  ...
                  max={1}
                  label={`${t(dict, 'countryFieldLabel')}`}
                //placeholder={`${t('Type to add tag')}...`}
                />
              </FormGroup>
            </Col>
            <Col className='mt-5'>
              <LanguageSelectMultiple onSelectLanguage={onSelectLanguage} defaultValue={formValues.languages} label={t(dict, 'userLanguage')} />
            </Col>
          </Row>
          <Row className="mt-5">
            <Col>
              <TagsInputMaterial tags={tags} max={5} setTags={setTags} label={t(dict, 'TopicsProfile')} className="mb-5" />
            </Col>
          </Row>
          <Row className="mt-4">
            <Col className="">
              <FormGroup controlId="aboutMe">
                <FormLabel>{t(dict, 'About me')}</FormLabel>
          ...
          <Row className='mt-5'>
            <FormControl>
              <FormLabel id="privacy-settings-label-placement">{t(dict, 'Privacy settings')}</FormLabel>
              ...
              >
                <FormControlLabel value={1} control={<Radio />} label={t(dict, 'My Mediatheque is public')} />
                <Form.Text className="ms-4">{t(dict, 'Anyone can see my Mediatheque')}.</Form.Text>
                <FormControlLabel value={2} control={<Radio />} label={t(dict, 'Fallowers can see my Dashboard')} />
                <Form.Text className="ms-4"> {t(dict, 'Fallowers can see my Dashboard')}.</Form.Text>
                <FormControlLabel value={3} control={<Radio />} label={t(dict, 'My Dashboard is secret')} />
                <Form.Text className="ms-4">{t(dict, 'Only I can see my Dashboard')}.</Form.Text>
              </RadioGroup>
            </FormControl>
          </Row>
