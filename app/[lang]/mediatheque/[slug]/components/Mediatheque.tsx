...
const Mediatheque:FC<Props>=({session})=>{
    ...
    const{data:readOrWatchedWorks}=useReadOrWatchedWorks(id);
    const{dict,t}=useDictContext();
    ...
    const getReadOrWatchedTotal = () => {
        if (readOrWatchedWorks) return readOrWatchedWorks.length;
    };

    const getReadCurrentYear = () => {
        if (readOrWatchedWorks) {
            if (readOrWatchedWorks.length) return readOrWatchedWorks.filter(x => ['book', 'fiction-book'].includes(x.work!.type) && x.year === dayjs().year()).length;
            else return 0;
        }
    };
    ...
    const getWatchedCurrentYear = () => {
        if (readOrWatchedWorks) {
            if (readOrWatchedWorks.length)
                return readOrWatchedWorks.filter(
